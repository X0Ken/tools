...
docker run \
    ...
    {{end}}--pid {{printf "%q" .HostConfig.PidMode}} \
    --ipc {{printf "%q" .HostConfig.IpcMode}} \
    {{printf "%q" .Config.Image}} \
    {{range .Config.Cmd}}{{printf "%q" .}} {{end}}
EOF

docker inspect --format "$(</tmp/docker-run.tpl)" "$container_name_or_id"
rm /tmp/docker-run.tpl
